...
We are not relying on the prompt, but a termination indicator,
The prompt is only required to detect when the initial connexion is ready, e.g. the prompt shows up.
